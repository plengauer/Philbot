...
async function requestConfig(config = null) {
    ...
    return new Promise((resolve, reject) => request.post({ url: 'http://' + (process.env.SHARDS_MASTER_HOST ?? '127.0.0.1') + ':' + (process.env.SHARDS_MASTER_PORT ?? '7999') + '/gateway/config', headers: { 'x-authorization': process.env.DISCORD_API_TOKEN }, body: body }, (error, response, body) => {
