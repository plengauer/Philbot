...
async function handle() {
  try {
    let metadata = JSON.parse(await curl.request({ secure: false, hostname: '169.254.169.254', path: '/latest/dynamic/instance-identity/document', cache: 60 * 60 * 24 })); // they always respond with plain/text
