...
async function handle() {
    return {
        ...
        body: '<html><head><title>Philbot HTTP Cache</title></head><body>'
            ...
            + curl.cache_summary().map(entry => `<tr><td>${sanitize(entry.key)}</td><td>${entry.hits}</td><td>` + Math.ceil((entry.timestamp + entry.ttl * 1000 - Date.now()) / 1000) + `</td><td>${entry.value?.status}</td><td>` + Object.keys(entry.value?.headers ?? {}).map(name => `${name}: ${entry.value.headers[name]}`).join('<br/>') + `</td><td>${entry.value?.body.length}</td></tr>`)
            + '</table>'
            + '</body></html>'
    }
}
