...
async function handle0(guild_id, user_id, activities) {
  ...
  return Promise.all([
      ...
      features.isActive(guild_id, "ranked game roles").then(active => active ? Promise.all(activities.map(a => games.updateRankedRoles(a.name, guild_id, user_id))) : Promise.resolve())
    ]);
}
