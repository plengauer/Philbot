...
async function forward(mirror_info, guild_id, channel_id, user_id, action, content, attachments, embeds, components, message_id, referenced_message_id) {
  ...
  while (content.includes('<@')) {
    ...
    let mentioned_member = await discord.guild_member_retrieve(guild_id, mentioned_user_id).then(member2string)
      .catch(() => discord.user_retrieve(mentioned_user_id).then(discord.user2name))
      .catch(() => discord.webhook_retrieve(mentioned_user_id).then(webhook => webhook.name))
      .catch(() => '<UnknownUser>');
  ...
  for (let index = 0; index < attachments.length; index++) {
    ...
    try {
      ...
      let file = await curl.request({ secure: attachment.url.startsWith('https://'), hostname: uri.hostname, port: uri.port, path: uri.pathname + (uri.search ?? ''), stream: true });
