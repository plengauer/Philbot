...
async function getPublicURL() {
  return new Promise((resolve, reject) => process.env.PUBLIC_URL ? resolve(process.env.PUBLIC_URL) : reject(new Error()))
    .then(url => curl.request({ secure: false, method: 'GET', hostname: url.substring(url.indexOf('://') + 3), path: '/', cache: 60 }).then(result => new Promise((resolve, reject) => result.includes('<title>Philbot</title>') ? resolve(url) : reject(new Error()))))
    .catch(error =>
