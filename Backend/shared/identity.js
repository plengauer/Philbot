...
async function getPublicURL() {
  return new Promise((resolve, reject) => process.env.PUBLIC_URL ? resolve(process.env.PUBLIC_URL) : reject(new Error()))
    ...
    .catch(error =>
      curl.request_simple({ secure: false, method: 'GET', hostname: '169.254.169.254', path: '/latest/meta-data/public-hostname', headers: { accept: 'text/plain' }, timeout: 100, fail_on_timeout: false, cache: 60 * 60 * 24 })
