...
async function GATEWAY_HTTP(endpoint, method, guild_id, payload = undefined, ttc = undefined) {
  ...
  return curl.request({ secure: true, method: method, hostname: url.parse(callback_url).hostname, port: url.parse(callback_url).port, path: endpoint, headers: { 'authorization': process.env.DISCORD_API_TOKEN }, body: payload, cache: ttc ?? 10 });
}
