...
async function on_message_create(guild_id, channel_id, message_id, user_id, content) {
  ...
  if (chatgpt.compareLanguageModelByPower(model_fast, 'gpt-4')) {
    let is_target_language = await chatgpt.createBoolean(user_id, `Is the text "${content}" ${target_language}?`, model_fast, 0);
    //console.log(`DEBUG TRANSLATOR v2 #1 "${content}" => ${is_target_language}`);
    if (is_target_language) return;
  } else {
    let target_language_percentage = await chatgpt.createResponse(user_id, `What percentage of "${content}" is ${target_language}? Respond only with the percentage.`, model_fast, 0);
  ...
  let translation = await translate(user_id, target_language, content, model)
...
async function translate(user, target_language, source, model = undefined) {
