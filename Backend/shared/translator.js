...
async function on_message_create(guild_id, channel_id, message_id, content) {
  ...
  if (false) {
    ...
    let target_language_percentage = await chatgpt.getResponse(null, null, `What percentage of "${content}" is ${target_language}? Respond only with the percentage.`, "gpt-3.5-turbo");
  ...
  if (source_language.toLowerCase().split(',').every(language => [target_language.toLowerCase().trim(), 'internet slang', 'discord mention', 'discord mentions', 'emoji', 'emojis'].includes(language))) return;
