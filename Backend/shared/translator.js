...
async function on_message_create(guild_id, channel_id, message_id, content) {
  ...
  if (source_language.endsWith('.') || source_language.split(',').some(language => language.split(' ').filter(token => token.length > 0).length > 3)) throw new Error('Invalid language: ' + source_language);
